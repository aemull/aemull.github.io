...
function do_attack ($iv_string_b64, $token, $url) {
	...
	for ($padding = 1; $padding <= 16; $padding++) {
		...
		for ($i = 0; $i <= 0xff; $i++) {
			...
			print "Trying IV: " . byte_array_to_string ($iv) . "\n";
			try {
	...
	*/
	...
	for ($i = 0; $i < strlen($new_clear); $i++) {
		...
		print "Zeroing array after change for byte $i: " . byte_array_to_string ($zeroing) . "\n";
	}
	$padding = 16 - strlen($new_clear);
	$offset = 16 - $padding;
	for ($i = $offset; $i < 16; $i++) {
		$zeroing[$i] = $zeroing[$i] ^ $padding;
		print "Zeroing array after change for padding byte $i: " . byte_array_to_string ($zeroing) . "\n";
	}
